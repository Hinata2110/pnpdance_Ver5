...
include CC_ROOT_DIR.CC_DS.'classes'.CC_DS.'autoloader.class.php';
...
function chmod_writable() {
	if (!defined('CC_CHMOD')) {
		...
		if (is_writable(CC_CACHE_DIR)) {
			$tmpdir = CC_CACHE_DIR.CC_DS.'chmodtmp';
...
function dirsize($path, &$total) {
	$path	= (substr($path, -1, 1) == CC_DS) ? $path : $path.CC_DS;
...
function findFiles(&$list, $path = false, $recursive = true) {
	$path .= (substr($path, -1) == CC_DS) ? '' : CC_DS;
	if (file_exists($path)) {
...
function offline() {
	...
	if ($GLOBALS['config']->get('config', 'offline')) {
		...
		if (!Admin::getInstance()->is() || (Admin::getInstance()->is() && !$GLOBALS['config']->get('config', 'offline_allow_admin'))) {
...
function state_json() {
	...
	if (($json = $GLOBALS['cache']->read('json.states')) === false) {
		$counties = $GLOBALS['db']->query('SELECT gc.numcode, gz.id, gz.name FROM `'.$GLOBALS['config']->get('config', 'dbprefix').'CubeCart_geo_zone` AS `gz` LEFT JOIN `'.$GLOBALS['config']->get('config', 'dbprefix').'CubeCart_geo_country` AS `gc` ON gc.id=gz.country_id');
		$json_array = array();
		if ($counties) {
			$current = '';
...
function rootHomePath() {
	return str_replace(array('public_html', 'htdocs'), '', str_replace(substr(str_replace("/",CC_DS,$GLOBALS['rootRel']),0,-1), '', CC_ROOT_DIR));
}
