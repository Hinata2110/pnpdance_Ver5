 ...
 ********************************************************************************/
	...
	if(!empty($_POST['submit'])){
		...
		if(empty($username) || empty($password)){
			...
			$check_result  = $hasher->CheckPassword($password, $mf_settings['admin_password']);
			if($check_result){
				$password_is_valid = true;
			}

			if($password_is_valid && (strtolower($username) == strtolower($mf_settings['admin_login']))){
				$_SESSION['mf_logged_in'] = true;

				//if the user select the "remember me option"
				//set the cookie and make it active for the next 30 days
				if(!empty($remember_me)){
					$cookie_hash = $hasher->HashPassword(mt_rand()); //generate random hash and save it into ap_settings page

					$query = "update ".MF_TABLE_PREFIX."settings set cookie_hash=?";
		   			$params = array($cookie_hash);
