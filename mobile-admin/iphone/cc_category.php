...
function getCategory($OnlyYes, $Filter, $CID, $ROW, $NUM_ROW) {
	...
	if ($select) {
		...
		while($row = @mysql_fetch_array($select)) {
			...
			$categories[category_parent_id]			= (string)$row[cat_father_id];
...
function updateCategory($CID, $Field, $Value) {
	...
	if ($Field == 'category_name') {
		...
		$query	= "UPDATE "._DB_PREFIX_.NAME_SHOP."_category SET cat_father_id='$Value' WHERE cat_id=" . $CID;
		$update	= @mysql_query($query);
		if ($update)	{ $Out	= ',"category_id":"'.$CID.'","query":"1"'; }
		else			{ $Out	= ',"query":"-1"'; }
	}
...
function deleteCategory($CID) {
	$CID	= mysql_real_escape_string($CID);
	$query	= "SELECT productId FROM "._DB_PREFIX_.NAME_SHOP."_cats_idx WHERE cat_id=$CID";
	$select	= @mysql_query($query);
	if (@mysql_num_rows($select) > 0) {
		$Out	= ',"query":"-2"';
	} else {
		
		$query	= "SELECT cat_id FROM "._DB_PREFIX_.NAME_SHOP."_category WHERE cat_father_id=" . $CID;
		$select	= @mysql_query($query);
