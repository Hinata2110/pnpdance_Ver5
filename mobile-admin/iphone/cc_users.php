...
function getUser($Filter, $UID="", $ROW=0, $NUM_ROW=0) {
	...
	if (!empty($Filter))	{ $query .= " AND (firstName LIKE '%$Filter%' OR lastName LIKE '%$Filter%' OR email LIKE '%$Filter%')"; }
	...
	if ($select) {
		...
		if ($rows > 0) {
			...
			while($row = @mysql_fetch_array($select)) {
				...
				$users[cdate]			= (string)$row[regTime];
				$users[firstname]		= (string)$row[firstName];
				$users[lastname]		= (string)$row[lastName];
