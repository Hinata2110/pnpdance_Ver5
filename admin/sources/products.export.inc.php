...
if (isset($_GET['format']) && !empty($_GET['format'])) {
    ...
    if ($results = $GLOBALS['db']->query($query, $per_page, $page)) {
		switch (strtolower($_GET['format'])) {
			case 'googlebase':
			case 'storeya':
				$header_fields	= array('id','product_type','link','title','description','image_link','price','condition','shipping_weight','upc','ean','jan','isbn','brand','availability');
				$fields		= array('product_id', 'google_category', 'url', 'name', 'description', 'image', 'price', 'condition','product_weight','upc','ean','jan','isbn','manufacturer','availability');
				$delimiter	= "\t";
				$extension	= 'txt';
				$glue		= "\r\n";
				$field_wrapper = '"';
			...
			case 'shopzilla':
				...
				break;
			case 'shopping.com':
				$header_fields	= array('mpn','upc','manufacturer','product name','product description','price','stock','stock description','product url','image url','category');
				$fields		= array('product_code', 'upc', 'manufacturer', 'name', 'description', 'price', 'stock', '', 'url', 'image', '');
				$delimiter	= ',';
				$extension	= 'txt';
				$glue		= "\n";
				$field_wrapper = '"';
				...
				break;
			case 'cubecart':
				$header_fields	= array('Product Name','Product Code','Product Description','Price','Sale Price','Image','Stock Level','Use Stock','Master Category ID','Manufacturer','UPC','EAN','JAN','ISBN');
				$fields		= array('name', 'product_code', 'description', 'price', 'sale_price', 'image', 'stock_level', 'use_stock_level', 'cat_id', 'manufacturer','upc','ean','jan','isbn');
				$delimiter	= ',';
				$extension	= 'csv';
				$glue		= "\n";
				$field_wrapper = '"';
