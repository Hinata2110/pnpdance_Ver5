	...
	if (isset($_POST['option_add']) && is_array($_POST['option_add']) && !empty($_POST['option_add'])) {
		foreach ($_POST['option_add']['value'] as $offset => $value) {
			$record	= array(
				...
				//'option_stock'		=> $_POST['option_add']['stock'][$offset],
			);
			if ($value > 0) {
				// get the option id
...
if (isset($_GET['delete']) && !empty($_GET['delete']) && Admin::getInstance()->permissions('products', CC_PERM_DELETE)) {
	/* !Delete Product */
	foreach ($GLOBALS['hooks']->load('admin.product.delete') as $hook) include $hook;
	if ($GLOBALS['db']->delete('CubeCart_inventory', array('product_id' => $_GET['delete']))) {
		// Delete category index
		$GLOBALS['db']->delete('CubeCart_category_index', array('product_id' => $_GET['delete']));
		// Delete product options
		$GLOBALS['db']->delete('CubeCart_option_assign', array('product' => $_GET['delete']));
		$GLOBALS['db']->delete('CubeCart_reviews', array('product_id' => $_GET['delete']));
		$GLOBALS['seo']->delete('prod',$_GET['delete']);
...
if (isset($_GET['action'])) {
	...
	if (strtolower($_GET['action']) == 'delete' && Admin::getInstance()->permissions('products', CC_PERM_DELETE)) {
		...
		if (($groups = $GLOBALS['db']->select('CubeCart_option_group')) !== false) {
			foreach ($groups as $group) {
				$group_list[$group['option_id']]	= $group;
			}
			if (($values = $GLOBALS['db']->select('CubeCart_option_value')) !== false) {
...
//		if (in_array(strtolower($_GET['action']), array('clone','edit')) && is_numeric($_GET['product_id'])) {
