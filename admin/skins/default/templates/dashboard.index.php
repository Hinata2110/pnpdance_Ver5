	...
	<form action="?_g=products&node=reviews&origin=dashboard" method="post" enctype="multipart/form-data">
	  ...
	  <div class="note">
		...
		<div class="details">
		  ...
		  <a href="?_g=products&amp;product_id={$review.product_id}&amp;action=edit">{$review.product.name}</a> &raquo; {$review.date} :: {$review.name}
		</div>
	  </div>
	  {/foreach}
	  <div>
...
<div id="stock_warnings" class="tab_content">
  ...
  {foreach from=$STOCK item=warn}
	<div>
	  <span class="actions"><a href="?_g=products&amp;action=edit&amp;product_id={$warn.product_id}" class="edit"><img src="{$SKIN_VARS.admin_folder}/skins/{$SKIN_VARS.skin_folder}/images/edit.png" alt="{$LANG.common.edit}" /></a></span>
	  <a href="?_g=products&amp;action=edit&amp;product_id={$warn.product_id}#product_stock">{$warn.name}</a> ({$LANG.dashboard.stock_level}: {$warn.stock_level})
	</div>
  {/foreach}
  </div>
