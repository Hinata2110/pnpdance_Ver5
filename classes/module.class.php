		...
		if ($path) {
			...
			if (isset($_POST['module']) && is_array($_POST['module'])) {
				...
				if ($this->module_settings_save($_POST['module'])) {
					$GLOBALS['main']->setACPNotify(sprintf($GLOBALS['language']->notification['notify_module_settings'], str_replace('_',' ',$this->_module_name)));
				} else {
					$GLOBALS['main']->setACPWarning(sprintf($GLOBALS['language']->notification['error_module_settings'], str_replace('_',' ',$this->_module_name)));
				}
				$this->_module_data($path, $local_name);
...
/*
			if (isset($_POST['zones']) && is_array($_POST['zones'])) {
				...
				$settings['countries']	= (isset($zones)) ? serialize($zones) : '';
			} else {
				$settings['countries']	= '';
			}
*/
...
//			$GLOBALS['smarty']->assign('MODULE_ZONES', $GLOBALS['smarty']->fetch('templates/modules.zones.php'));
