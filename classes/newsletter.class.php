	...
	public function saveNewsletter($newsletter = false) {
		...
		if (!empty($newsletter) && is_array($newsletter)) {
			if (!empty($newsletter['newsletter_id']) && is_numeric($newsletter['newsletter_id'])) {
				$GLOBALS['db']->update('CubeCart_newsletter', $newsletter, array('newsletter_id' => $newsletter['newsletter_id']));
				$this->_newsletter_id = $newsletter['newsletter_id'];
			} else {
				$GLOBALS['db']->insert('CubeCart_newsletter', $newsletter);
			...
			if (($contents = $GLOBALS['db']->select('CubeCart_newsletter', false, array('newsletter_id' => (int)$newsletter_id))) !== false) {
				...
				if ($test) {
					...
					if (filter_var($test, FILTER_VALIDATE_EMAIL)) {
						//$content['content_html'] = $this->_html->display(true, true, true);
						//$content['content_text'] = $this->_text->display(true, true, true);
					...
					if (($subscribers = $GLOBALS['db']->select('CubeCart_newsletter_subscriber', array('email'), array('status' => '1'), false, $limit, $cycle)) !== false) {
						foreach ($subscribers as $subscriber) {
							...
							/* Start fixing bug no: 2869 */
							/*	// HTML content
								$this->_html->assign('USER', $subscriber);
								// Text content
								$this->_text->assign('USER', $subscriber);
								// Output content
								$this->_html->parse();
								$this->_text->parse();
								$content	= array(
									'subject'		=> $content['subject'],
									'content_html'	=> $this->_html->display(true, true, true),
									'content_text'	=> $this->_text->display(true, true, true),
								);*/
								$content	= array(
									'subject'		=> $content['subject'],
