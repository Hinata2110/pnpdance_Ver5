...
class Catalogue {
	...
	public function getProductData($product_id, $quantity = 1, $order = false, $per_page = 10, $page = 1, $category = false) {
...
//			$where = $GLOBALS['db']->where('CubeCart_inventory', array('product_id' => $product_id, 'status' => 1));
	...
	public function getProductOptions($product_id = null) {
		if (($setlist = $GLOBALS['db']->select('CubeCart_options_set_product', array('set_id'), array('product_id' => (int)$product_id))) !== false) {
			...
			foreach ($setlist as $set_data) {
				if (($sets = $GLOBALS['db']->select('CubeCart_options_set_member', false, array('set_id' => (int)$set_data['set_id']))) !== false) {
					...
					if (($groups = $GLOBALS['db']->select('CubeCart_option_group', false, array('option_id' => $set_groups), array('priority' => 'DESC', 'option_name' => 'ASC'))) !== false) {
		...
		if (($products = $GLOBALS['db']->select('CubeCart_option_assign', false, array('product' => (int)$product_id, 'set_member_id' => 0, 'set_enabled' => '1'))) !== false) {
			...
			if (($categories = $GLOBALS['db']->select('CubeCart_option_group', array('option_id', 'option_name', 'option_type', 'option_required'), array('option_id' => $top), array('priority' => 'DESC', 'option_name' => 'ASC'))) !== false) {
				foreach ($categories as $category) {
					$array = false;
	...
	public function getProductStock($product_id = null, $option_id = null) {
						...
						case self::OPTION_TEXTAREA:		## Textarea option
							foreach ($group as $key => $option) {
								$option_list[] = array(
									...
									'price'			=> (isset($option[0]['option_price'])) ? $GLOBALS['tax']->priceFormat($option[0]['option_price'], true) : false,
