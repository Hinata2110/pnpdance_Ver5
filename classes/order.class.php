...
class Order {
	...
	public function createOrderId($return = false) {
	...
	private function _manageStock($status_id, $order_id) {
		...
		if (($items = $GLOBALS['db']->select('CubeCart_order_inventory', false, array('cart_order_id' => $order_id), false, false, false, false)) !== false) {
			foreach ($items as $item) {
				if (($product = $GLOBALS['db']->select('CubeCart_inventory', array('use_stock_level', 'stock_level'), array('product_id' => (int)$item['product_id']), false, false, false, false)) !== false) {
					if ($product[0]['use_stock_level']) {
