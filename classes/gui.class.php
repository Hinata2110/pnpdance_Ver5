...
class GUI {
	...
	public function addBreadcrumb($name, $url = array()) {
		...
		if (!empty($name)) {
			$this->_breadcrumb[] = array(
				...
				'title'	=> ucwords(strip_tags(str_replace('_',' ',$name)))
			);
		}
	}
	...
	private function _displayRandomProduct($i=0) {
		...
		} elseif($i<15) { // Math to generate random product might give a product_id that doesn't exist or doen't meet WHERE criteria
			$i++;
			$this->_displayRandomProduct($i);
		}
	}
