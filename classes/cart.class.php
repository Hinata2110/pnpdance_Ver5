		...
		if (!empty($this->basket['contents']) && is_array($this->basket['contents'])) {
			...
			foreach ($this->basket['contents'] as $hash => $item) {
				...
				if (is_numeric($item['id'])) {
					$product = $GLOBALS['catalogue']->getProductData($item['id'], $item['quantity']);
				...
				if (isset($tax_state_id)) {
					$GLOBALS['tax']->productTax($product['price'], (int)$product['tax_type'], (bool)$product['tax_inclusive'], $tax_state_id);
				} else {
					$GLOBALS['tax']->productTax($product['price'], (int)$product['tax_type'], (bool)$product['tax_inclusive']);
				}
		...
		if (isset($_POST['quan']) && is_array($_POST['quan'])) {
			...
			foreach ($_POST['quan'] as $hash => $quantity) {
				if ($quantity <= 0) {
					...
					if ($this->basket['contents'][$hash]['quantity'] != $quantity) {
