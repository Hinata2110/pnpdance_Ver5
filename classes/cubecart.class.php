	...
	private function _checkout() {
		...
		if (!$GLOBALS['user']->is()) {
			...
			if (isset($_POST['user']) && isset($_POST['billing'])) {
				...
				if (isset($_POST['register']) && !empty($_POST['password']) && $_POST['password'] !== $_POST['passconf']) {
				...
				if (!isset($errors)) {
					...
					$_POST['user']['password'] = (isset($_POST['register'])) ? md5($_POST['password']) : md5(time().$SERVER['HTTP_USER_AGENT'].$_SERVER['REMOTE_ADDR']);
					$type = isset($_POST['register']) ? 1 : 2;
					...
					// Log in
					if (isset($_POST['register']) && !$GLOBALS['user']->authenticate($_POST['user']['email'], $_POST['password'], false, false, false, false)) {
...
//			$this->_basket['shipping_verified'] = true;
	...
	private function _checkoutProcess($section = null) {
		...
		// Language fixed on other area this is not required.
		//$GLOBALS['language']->loadLang(); // Reload stuff otherwise standard English remains for a while
	...
	private function _complete() {
		...
		if (isset($this->_basket['cart_order_id'])) {
			...
			if (($orders = $GLOBALS['db']->select('CubeCart_order_summary', false, array('cart_order_id' => $this->_basket['cart_order_id']))) !== false) {
				...
				$GLOBALS['smarty']->assign('ANALYTICS', ($GLOBALS['config']->get('config', 'google_analytics') && !COOKIE_BLOCK) ? true : false);
	...
	private function _displayBasket($editable = true) {
		...
		if ($contents) {
			...
			if (($shipping = $GLOBALS['cart']->loadShippingModules()) !== false) {
				...
				foreach ($shipping as $ship_name => $methods) {
					...
					foreach ($methods as $data) {
						...
						$option	= array(
							'value'		=> base64url_encode(serialize($value)),
						...
						if(isset($this->_basket['shipping']) && $this->_basket['shipping']['offset'] == $offset) {
							...
							if($value['value'] !== $this->_basket['shipping']['value']) {
			...
			if (isset($this->_basket['coupons']) && is_array($this->_basket['coupons']) || !empty($this->_basket['discount'])) {
				if (!empty($this->_basket['discount'])) {
	...
	private function _displayGateways($name = false) {
		...
		$gateways = $GLOBALS['db']->select('CubeCart_modules', false, $where);
		// Gateway hooks
		...
		if (isset($_GET['newsletter_id']) && is_numeric($_GET['newsletter_id'])) {
			...
			if ($GLOBALS['user']->is()) {
				...
				$GLOBALS['smarty']->assign('URL', array(
					'subscribe' => currentPage(array('subscribed'), array('action' => 'subscribe')),
					'unsubscribe' => currentPage(array('subscribed'), array('action' => 'unsubscribe'))
				));
	...
	private function _orders() {
		...
		if ($GLOBALS['user']->is()) {
			...
			if (isset($_GET['cart_order_id']) && preg_match('#^[0-9]{6}-[0-9]{6}-[0-9]{4}$#i', trim($_GET['cart_order_id']))) {
				...
				$per_page = 15;
				$page = (isset($_GET['page'])) ? $_GET['page'] : 1;
				if (($orders = $GLOBALS['db']->select('CubeCart_order_summary', array('cart_order_id', 'ship_tracking', 'order_date', 'status', 'total'), array('customer_id' => $GLOBALS['user']->get('customer_id')), array('cart_order_id' => 'DESC'), $per_page, $page)) !== false) {
					foreach ($orders as $i => $order) {
						...
						$order['make_payment']	= ($order['status'] == 1) ? true : false;
