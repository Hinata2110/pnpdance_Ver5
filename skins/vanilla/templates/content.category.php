...
<form action="{$VAL_SELF}" method="post" enctype="application/x-www-form-urlencoded" class="addForm">
  <div class="category_product">
	...
	{if $product.ctrl_sale}
	  <p class="price"><span class="price_previous">{$product.price}</span><br /><span class="price_sale">{$product.sale_price}</span></p>
	{else}
	  <p class="price">{$product.price}</p>
	{/if}
	{if $product.ctrl_purchase && !$CATALOGUE_MODE}
	  <p class="buy_button"><input type="text" name="add[{$product.product_id}][quantity]" value="1" class="quantity" /> <input type="submit" value="{$LANG.catalogue.buy}" class="button_buy" /></p>
	{elseif $product.out}
	  <p class="out_of_stock">{$LANG.catalogue.out_of_stock_short}</p>
	{/if}
	</div>
	<div class="summary">
		...
		<p class="description">{$product.description_short} <a href="{$product.url}" title="{$product.name}">{$LANG.common.more_info}</a></p>
	</div>
