...
if ($homepage = $db->select('CubeCart_lang', false, array('identifier' => '/'.$short_lang_identifier.'/home.inc.php'))) {
	...
	$lang_path = CC_ROOT_DIR.CC_DS.'language'.CC_DS.$short_lang_identifier.CC_DS.'home.inc.php';
	if(file_exists($lang_path)) {
		...
		$record	= array(
				...
				'doc_home'		=> 1,
				'doc_name'		=> $home['title'],
				'doc_content'	=> $home['copy'],
			);
		$db->insert('CubeCart_documents', $record);
	}
}

$from	= CC_ROOT_DIR.CC_DS.'images'.CC_DS.'uploads';
$to		= CC_ROOT_DIR.CC_DS.'images'.CC_DS.'source';
...
if (file_exists($from) && is_writable(dirname($from))) {
	if (!file_exists($to) && rename($from, $to)) {
		...
		recursiveDelete($to.CC_DS.'thumbs');
	} else {
...
update_image_paths(CC_ROOT_DIR.CC_DS.'images'.CC_DS.'source'.CC_DS.'*');
...
foreach (glob('..'.CC_DS.'images'.CC_DS.'logos'.CC_DS.'*') as $file) {
...
$GLOBALS['db']->parseSchema(file_get_contents('db'.CC_DS.'install'.CC_DS.'email.sql', false));
