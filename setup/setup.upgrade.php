...
if (!isset($_SESSION['setup']['permissions'])) {
	...
	if (($versions = $db->select('CubeCart_history', array('version'), null, array('id' => 'DESC'), false, false, false)) !== false) {
		...
		$v3_ini	= CC_ROOT_DIR.CC_DS.'includes'.CC_DS.'ini.inc.php';
	...
	if (!isset($_SESSION['setup']['progress']) || is_null($_SESSION['setup']['progress'])) {
		...
		$files_sql = glob($setup_path.'db'.CC_DS.'upgrade'.CC_DS.'*.sql');
		if ($files_sql) {
		...
		$files_php = glob($setup_path.'scripts'.CC_DS.'upgrade'.CC_DS.'*.php');
		if ($files_php) {
		...
		if (is_array($updates) && !empty($updates)) {
			...
			foreach ($updates as $version) {
				...
				$file_sql	= 'db'.CC_DS.'upgrade'.CC_DS.$version.'.sql';
				if (file_exists($file_sql)) {
					## Process file
					$db->parseSchema(file_get_contents($file_sql, false));
				}

				$file_php	= 'scripts'.CC_DS.'upgrade'.CC_DS.$version.'.php';
